...
export default function ReviewQueuePage() {
  ...
  const sendTelegramNotify = async (
    eventType: string,
    opts: Record<string, string | number | boolean | null | undefined>,
  ) => {
    try {
      ...
      await writeAppLog({
        ...
        message: error instanceof Error ? error.message : "Telegram notify failed",
        path: "/admin/review",
        userId: user?.id ?? null,
        context: { eventType, ...opts },
      });
    }
  };
          ...
          <DataTable
            ...
            columns={[
              ...
              },
              {
                ...
                render: (r: RequestRow) => <span className="text-sm text-muted-foreground">{new Date(r.created_at).toLocaleDateString("uz")}</span>,
              },
              {
