...
export default function CommentsPage() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    let query = supabase.from("comments").select("*, contents(title), profiles:user_id(full_name, avatar_url)").order("created_at", { ascending: false }).limit(200);
    ...
    <div className="animate-fade-in">
      ...
      <Tabs value={tab} onValueChange={setTab} className="space-y-4">
        ...
        <DataTable data={filtered} loading={loading} emptyMessage="Izohlar topilmadi" columns={[
          { key: "user", header: "Foydalanuvchi", render: (c: any) => (
            <div className="flex items-center gap-2">
              {c.profiles?.avatar_url ? <img src={c.profiles.avatar_url} className="h-6 w-6 rounded-full" /> : <div className="h-6 w-6 rounded-full bg-secondary flex items-center justify-center text-xs">{c.profiles?.full_name?.[0] || "?"}</div>}
              <span className="text-sm text-foreground">{c.profiles?.full_name || "Noma'lum"}</span>
            </div>
          )},
          { key: "body", header: "Izoh", render: (c: any) => <p className="text-sm text-muted-foreground truncate max-w-[300px]">{c.body}</p> },
          { key: "content", header: "Kontent", render: (c: any) => <span className="text-sm text-muted-foreground">{c.contents?.title || "—"}</span> },
          { key: "likes", header: "👍/👎", render: (c: any) => <span className="text-xs text-muted-foreground">{c.like_count}/{c.dislike_count}</span> },
          { key: "created_at", header: "Sana", render: (c: any) => <span className="text-xs text-muted-foreground">{new Date(c.created_at).toLocaleDateString("uz")}</span> },
          { key: "actions", header: "", className: "w-28", render: (c: any) => (
