...
async function createEpisodeRequestFromTelegram({
  ...
  let duplicatePendingQuery = supabase
    ...
    .eq("status", "pending");
  duplicatePendingQuery = target.season?.id
    ? duplicatePendingQuery.eq("season_id", target.season.id)
    : duplicatePendingQuery.is("season_id", null);
  const { data: duplicatePending } = await duplicatePendingQuery.maybeSingle();
  ...
  let existingEpisodeQuery = supabase
    .from("episodes")
    .select("id")
    .eq("content_id", target.content.id)
    .eq("episode_number", parsed.episode_number);
  existingEpisodeQuery = target.season?.id
    ? existingEpisodeQuery.eq("season_id", target.season.id)
    : existingEpisodeQuery.is("season_id", null);
  const { data: existingEpisode } = await existingEpisodeQuery.maybeSingle();

  if (existingEpisode) {
    await writeAuditLog(supabase, {
      level: "warn",
      source: "telegram-bot",
      event: "episode_request_duplicate_episode",
      message: "Episode already exists",
      user_id: link.user_id,
      request_id: requestId,
      context: { contentId: target.content.id, seasonId: target.season?.id || null, episodeNumber: parsed.episode_number, episodeId: existingEpisode.id },
    });
    return sendFresh(api, chatId, "⚠️ Bu epizod allaqachon panelda mavjud\\.");
  }
